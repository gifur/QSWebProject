...
{
    public class QsFileAppender : RollingFileAppender
    {
        /// <summary>
        ...
        protected override void OpenFile(string fileName, bool append)
        {
            lock (this)
            {
                ...
                int newSuffix = Suffixes[completeFilename];
                ...
                for (int i = 1; !fileOpened && i <= Retries; i++)
                {
                    ...
                    {
                        newSuffix = Suffixes[completeFilename] + i;

                        LogLog.Error(typeof(QsFileAppender), string.Format("QsFileAppender: Failed to open [{0}]. Attempting [{1}-{2}] instead.", fileName, fileName, newSuffix));
                    }
                }
